...
CoverInfo Mapping::cal_CoverOfLine(geos::geom::LineString& ln1, geos::geom::LineString& ln2, double dis_Threshold)
{
...
WayFeature Mapping::calMappingFeature(geos::geom::LineString& ln1, geos::geom::LineString& ln2, double dist_Threshold)
{
	WayFeature wayFeature = WayFeature();
	CoverInfo ci = Mapping::cal_CoverOfLine(ln1, ln2, dist_Threshold);
	int num = 0;
	double dis_avg = 0;
	...
	for(int i=ci.startp_ln1_pos; i<=ci.endp_ln1_pos; i++)
	{
		geos::geom::Coordinate pt_ln1 = ln1.getCoordinateN(i);
		for( ; j<ln2.getNumPoints(); j++)
		{
	...
	for(int i=ci.startp_ln2_pos; i<=ci.endp_ln2_pos; i++)
	{
